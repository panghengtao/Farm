...
public class ShopCenterScript : MonoBehaviour
{
        ...
        if (availableRow.Count != 0)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                if (cusDatas[temRow, i].indexProduct == -1)//catch slot empty
                {
                    ...
                    {
                        GameObject.Find("UI Root").transform.FindChild("PanelCustomer").GetComponent<CustomerScript>().CreateCustomer(temRow, i, cus.typeCustomer, cus.indexProduct, true);
                    }
                    break;
                }
            }
    ...
    void isCompleteService(int row, int star, int money, bool isBonus = false)
    {
        ...
        if (Application.loadedLevelName.Equals("Store"))//when player's using shop scene.
        {
            GameObject.Find("UI Root").transform.FindChild("PanelCustomer").GetComponent<CustomerScript>().UpdatedRow(row, star + (DialogShop.BoughtItem[9] ? 1 : 0), money * (MissionPowerUp.DoubleMoney ? 2 : 1), isBonus);
        }
