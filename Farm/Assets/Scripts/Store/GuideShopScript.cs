...
public class GuideShopScript : MonoBehaviour
{
    ...
    {
        //delete old circles
        Transform[] tranforms = transform.FindChild("Circle").GetComponentsInChildren<Transform>();
    ...
    private void getCircleAndHand()
    {
        foreach (XmlNode child in textGuideXML.getDataByValue("id", indexGuide.ToString()).ChildNodes)
        {
            ...
            circle.name = "Circle";
            circle.transform.parent = this.transform.FindChild("Circle");
            ...
            if (child.Name.Equals("hand"))
            {
                ...
                hander.name = "Hand";
                hander.transform.parent = this.transform.FindChild("Circle");
                if (ranger > 0)
