...
public class DialogAchievement : DialogAbs
{
    ...
    void Awake()
    {
        ...
        grid = dialogMain.FindChild("Scroll View").FindChild("Grid").GetComponent<UIGrid>();
        ...
        btnAchievement = transform.parent.parent.FindChild("Button").FindChild("ButtonAchievement");
        lbCount = btnAchievement.FindChild("Count").FindChild("Label").GetComponent<UILabel>();
    ...
    public override void HideDialog(DialogAbs.CallBackHideDialog callback = null)
    {
        ...
        LeanTween.scale(dialogMain.gameObject, new Vector3(0, 0, 0), 0.4f).setEase(LeanTweenType.easeInBack).setOnComplete(() =>
        {
            ...
            Show = false;
            dialogMain.FindChild("Scroll View").FindChild("Grid").gameObject.SetActive(false);
