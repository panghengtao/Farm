...
public class TechnogyController : MonoBehaviour
{
    ...
    void CreateIcon()
    {
        ...
        for (int i = 0; i < iDChange.Count; i++)
        {
            ...
            icon[i].transform.FindChild("IconObject").GetComponent<UITexture>().mainTexture = listTextture[countTexture];
            icon[i].transform.FindChild("Label").GetComponent<UILabel>().text = TownScenesController.languageTowns[TownScenesController.ListTechnogyData[iDChange[i] - 1].name];
