...
public class CommonObjectScript : MonoBehaviour
{
    ...
    void OnApplicationPause(bool pauseStatus)
    {
        if (!pauseStatus)
        {
            ...
            if (isOpenStorage)
            {
                transform.FindChild("PanelStorage").GetComponent<StorageController>().Close_Click();
            }
            else if (isOpennew)
            {
                transform.FindChild("NewItemsPopup").GetComponent<PanelNewItemScript>().OK_Click();
            }
    ...
    {
        audioControl.PlaySound("Click 1");
        transform.FindChild("PanelStorage").gameObject.SetActive(true);
        isOpenStorage = true;
        if (isGuide) GameObject.Find("GuideFarmController").GetComponent<GuideFarmScript>().NextGuideText();
    }
